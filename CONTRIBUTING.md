...
CI runs the gates in `.github/workflows/kit-ci.yml` on every push and PR. Run them locally first:
...
# Script test suites (stdlib-only Python, run directly)
...
- Records are **append-only**: a decided record gets a dated amendment section (`## Amendment (YYYY-MM-DD)`), never a rewrite, so the reasoning trail stays legible.
