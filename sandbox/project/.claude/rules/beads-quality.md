...
**Authoring procedure lives in the `skills/bead-authoring` skill** (worked `bd create` examples, dedupe search, validation mechanics, wisps). This rule states the standards; the skill is how you meet them. Close-side law — evidence policy, do-not-defer, the commit/branch/PR reference requirement — lives in `bead-completion.md`; close-reason format and examples: the skill.
...
Use `bd create --acceptance "..."`; per-type requirements:
...
Dedupe search, skip-when conditions, and validation mechanics (`bd lint` / `bd create --dry-run`): see `skills/bead-authoring` Phases 1 and 3.
