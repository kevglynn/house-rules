...
**Status:** Draft. Publication-ready light pass complete. Not yet published.
...
Most AI policies say "we'll allow AI contributions if you follow these rules." The Covenant says AI-assisted development is a valid contributor profile, the governance is designed for it, and both sides of that deal get enforced.
...
So we added a **Disclosure Safe Harbor**: using someone's disclosure against them is a conduct violation. The stress-test surfaced a real concern: maintainers need to retain authority over code quality and architectural direction without fear of conduct complaints every time they reject an AI-assisted PR.
...
Copy the Covenant from [github.com/gastownhall/beads/blob/main/CODE_OF_CONDUCT.md](https://github.com/gastownhall/beads/blob/main/CODE_OF_CONDUCT.md). Customize the contact email and rate limits. Keep the attribution.

If you want the checkable playbook that sits under that governance layer (rules + stdlib checkers, no install required for the first demo), start here: [Try it without installing](../README.md#try-it-now-no-install).
