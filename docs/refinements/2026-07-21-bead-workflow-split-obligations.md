...
| BC10 | The evidence table (10 bead-type rows, Bug through Epic) | **Rule-BC** (kept verbatim, Epic row intact) — forward obligation 2 discharged: `skills/graph-planning` G7 points at the Epic row as canonical close-evidence law; not relocated or reworded. **Flagged: this table is the top profile candidate for the Wave 3 profile spike (process-kit-gsx)** — it is exactly the per-type enumerable-data shape the profile projection prototype (`experiments/profile-spike/profile.yaml`) already models as `close_evidence` |
