...
**Status:** Decided (2026-07-20 — all findings accepted; see §10)
...
hardcoded tag list includes another project's vocabulary (`swift swiftui appkit menubar`), violating the neutrality convention.
...
All four resolved at the 2026-07-20 gate:

- [x] Memory-layer direction: **retire** (per recommendation; reopen
  trigger recorded in §8). → bead process-kit-amq
- [x] bd server mode vs embedded: **ship the embedded-mode branch in the
  rules**; no server-mode migration. → bead process-kit-qxc
- [x] Scorecard activation: **re-scope to experiment windows** (matches its
  only real use). → bead process-kit-qxc
- [x] Cursor execution of `.claude/settings.json` hooks: **test
  empirically**. → spike bead process-kit-2k9
...
| ------------------- | -------- | --------- | ---------- | --------- |
| F1                  | accepted | kevglynn  | 2026-07-20 | Staged split program as recommended (Set 1, option 3); pilot method proven |
| F2                  | accepted | kevglynn  | 2026-07-20 | Drift is observed fact; destination skills exist; first Wave 2 split |
| F3                  | accepted | kevglynn  | 2026-07-20 | Dedupe as canonical homes are touched in Wave 2 |
| F4                  | accepted | kevglynn  | 2026-07-20 | Truth-in-labeling + cheap deterministic layer (Set 2, option 3) |
| F5                  | accepted | kevglynn  | 2026-07-20 | Retire; layer is dead and undocumented; reopen trigger in §8 |
| F6                  | accepted | kevglynn  | 2026-07-20 | Close drift channels via doctor/CI extensions (Wave 3) |
| F7                  | accepted | kevglynn  | 2026-07-20 | One-line fixes; embedded-mode branch chosen over server migration |
| F8                  | accepted | kevglynn  | 2026-07-20 | Retire orphaned imports; hygiene fixes (Wave 1) |
| F9                  | accepted | kevglynn  | 2026-07-20 | Extend orientation docs; drift pattern recorded |
| F10                 | accepted | kevglynn  | 2026-07-20 | Spike only (Set 3, option 3); full build gated on spike result |
| F11 (confirmations) | accepted | kevglynn  | 2026-07-20 | Recorded and preserved as reference patterns |

Approval given in-session ("i like it! let's go!!! your recos are good!"),
recorded by the executing agent per the proposal-only operating mode.

**Resulting work items:** three epics created 2026-07-20, wired via
`bd create --graph`:

- **Wave 1** `process-kit-6hx` — children: `amq` (retire memory layer, F5),
  `opk` (skills hygiene, F8), `qxc` (broken prescriptions, F7), `815`
  (docs orientation, F9), `2k9` (Cursor-hook spike, open Q4)
- **Wave 2** `process-kit-j4x` — serial chain: `k5m` (multi-agent-review
  split, F2) → `oub` (operating-model split, F1/F3) → `n0w`
  (bead-completion + beads-quality, F1) → `jp9` (defer-convention +
  defer-lint, F1/F4) → `3q1` (minor trims, F1)
- **Wave 3** `process-kit-iow` — children: `chd` (kit CI, F6), `loi`
  (doctor/distribution gaps, F6; blocked on `amq`), `gsx` (checkers,
  F4/F10; blocked on `jp9`), `vgz` (profile spike, F10)

**Traceability:** pre-change state is commit `1b62a74` (the commit
containing this proposal's appendix); the pilot proposal and lane evidence
(bead process-kit-l1h notes + session subagent transcripts) are the
analysis record. Status flipped to **Decided**.