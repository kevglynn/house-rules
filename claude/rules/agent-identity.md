...
**Checker:** `banned-token-scan` (kit-distributed; local installs also ship `scripts/banned-token-scan`) mechanizes this scan — reports candidate hits by token class for files or stdin (`--json` for agents); the not-regressions judgment stays with the reader. Class catalog and judgment boundary in `banned-token-scan --help`.
