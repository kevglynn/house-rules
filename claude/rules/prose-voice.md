...
Punctuation is part of voice: match the user's fingerprint, not model defaults. Per-user parameters (banned marks and their replacements, register rules) live in the machine-global user overlay (`~/.agents/overlay.md` § `prose-voice`) — the skill documents the keys it reads and applies the generic law when no overlay exists.
