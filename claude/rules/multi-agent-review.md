...
| `simplify` | Over-engineering, unnecessary complexity, premature abstraction. Uses structured tags + free-form structural reasoning | Gold-plating, dead code, abstractions without consumers, stdlib replacements, YAGNI violations |
...
### Simplify lens: structured output guidance

The simplify lens produces two sections in its output:

**Section 1 — Tagged findings (line-level).** One line per finding:
...
End the tagged section with: `net: -<N> lines possible.` If nothing to cut: `Lean already. Ship.`

**Section 2 — Structural observations (free-form).** After the tagged findings, note any structural concerns that don't reduce to a single line — patterns like "this wrapper shouldn't exist," "the test suite tests upstream behavior not this code," or "this is a tutorial masquerading as an implementation." These observations often represent the highest-value findings and must not be suppressed by the line-level format.

**Scope boundary:** Over-engineering and complexity only. Correctness bugs, security holes, and performance issues are explicitly out of scope — route them to the code-reviewer or architect-reviewer lens.
