...
**Checker:** `scripts/defer-lint` (kit-distributed) verifies the law — exit 1 on any defer lacking its upgrade-when clause; `--json` for agents; worked good/bad examples in `defer-lint --help`. Harvesting deferrals into a debt ledger: the `ponytail-debt` skill.
