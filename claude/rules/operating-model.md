...
- When Jawnt MCP is available, use it for cross-project reads — one MCP call, not a `bd` loop across directories; **all writes go through local `bd`** (the MCP tools are read-only).
