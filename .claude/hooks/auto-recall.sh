...
if [[ ! -d "$PROJECT_DIR/.beads/memory" ]]; then
  if [[ ! -f "$SCRIPT_DIR/provision-memory.sh" ]]; then
    cat << 'BROKEN'
{"hookSpecificOutput":{"systemMessage":"## Memory Bootstrap Failed\n\nCannot create `.beads/memory/` — `provision-memory.sh` is missing from the plugin directory. The beads-compound plugin may be partially installed. Re-run the installer to restore missing files."}}
BROKEN
    exit 0
  fi
